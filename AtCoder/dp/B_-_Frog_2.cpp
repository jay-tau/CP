...
int recursive_cost_fn(vi &h, int ptr, int k) {
  if (dp[ptr] != -1)
    return dp[ptr];
  ...
  fo1(i, k) {
    if (ptr + i < n) {
      max_cost = min(max_cost, abs(h[ptr] - h[ptr + i]) +
                                   recursive_cost_fn(h, ptr + i, k));
    }
  }
  return dp[ptr] = max_cost;
}
...
void solve() {
  ...
  vi v(n);
  cin >> v;
  cout << recursive_cost_fn(v, 0, k) << endl;
}
