...
void solve() {
  int n, m, a, b;
  cin >> n >> m >> a >> b;
  int cost_for_m_rides_single = m * a;
  if (cost_for_m_rides_single <= b) {
    cout << (n * a) << endl;
    return;
  } else {
    int rides = n / m;
    ...
    int cost_remaining_rides = remaining_rides * a;
    if (cost_remaining_rides <= b) {
      cost += cost_remaining_rides;
    } else {
      cost += b;
    }
    cout << cost << endl;
