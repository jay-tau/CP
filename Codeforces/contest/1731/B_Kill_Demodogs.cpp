...
void solve() {
    ...
    int n;
    cin >> n;
    // ll ans = ((n*(n+1)*(2*n+1))/6) + (((n-1)*n*(2*n-1))/6) + (((n-1)*n)/2);
    ll ans = modadd(modadd(moddiv(modmul(modmul(n, n+1, N), (2*n+1), N), 6, N), moddiv(modmul(modmul(n-1, n, N), (2*n-1), N), 6, N), N), moddiv(modmul(n-1, n, N), 2, N), N);
    cout << modmul(2022, ans, N) << endl;
}
