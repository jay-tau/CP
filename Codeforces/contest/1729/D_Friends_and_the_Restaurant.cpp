...
void solve() {
  ...
  cin >> n;
  int expenditure[n];
  fo(i, n) { cin >> expenditure[i]; }
  int income[n], surplus[n];
  fo(i, n) {
    cin >> income[i];
    surplus[i] = income[i] - expenditure[i];
  }

  sort(surplus, surplus + n);
  ...
  while (left_pointer < right_pointer) {
    if ((surplus[left_pointer] + surplus[right_pointer]) >= 0) {
      num_days++;
      left_pointer++;
      right_pointer--;
    } else {
      // If not enough money, skip the left_pointer, and move it ahead
      left_pointer++;
    }
  }
  cout << num_days << endl;
